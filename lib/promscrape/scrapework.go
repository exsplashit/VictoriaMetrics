...
import (
	...
	"github.com/exsplashit/VictoriaMetrics/lib/leveledbytebufferpool"
	...
	parser "github.com/exsplashit/VictoriaMetrics/lib/protoparser/prometheus"
	"github.com/exsplashit/VictoriaMetrics/lib/protoparser/prometheus/stream"
...
type ScrapeWork struct {
	...
	// See https://github.com/exsplashit/VictoriaMetrics/issues/3137
	//
	...
	for {
		...
		case <-stopCh:
			...
			case <-globalStopCh:
				...
				// See https://github.com/exsplashit/VictoriaMetrics/issues/2013#issuecomment-1006994079
			default:
	...
	if err != nil {
		...
		if err == nil {
			...
			err = stream.Parse(&sbr, scrapeTimestamp, false, func(rows []parser.Row) error {
				...
				// See https://github.com/exsplashit/VictoriaMetrics/issues/825#issuecomment-723198247
				sw.pushData(sw.Config.AuthToken, &wc.writeRequest)
				wc.resetNoRows()
				return nil
			}, sw.logError)
...
func (sw *scrapeWork) areIdenticalSeries(prevData, currData string) bool {
	if sw.Config.NoStaleMarkers && sw.Config.SeriesLimit <= 0 {
		...
		// The check for series_limit is needed for https://github.com/exsplashit/VictoriaMetrics/issues/3660
		return true
	}
...
func (sw *scrapeWork) sendStaleSeries(lastScrape, currScrape string, timestamp int64, addAutoSeries bool) {
	...
	// That's why it is a good idea to limit the number of concurrent calls to this function
	// in order to limit memory usage under high load without sacrificing the performance.
	// See https://github.com/exsplashit/VictoriaMetrics/issues/3668
	...
	if bodyString != "" {
		...
		// See https://github.com/exsplashit/VictoriaMetrics/issues/3668
		// and https://github.com/exsplashit/VictoriaMetrics/issues/3675
		var mu sync.Mutex
		...
		err := stream.Parse(br, timestamp, false, func(rows []parser.Row) error {
			...
			// See https://github.com/exsplashit/VictoriaMetrics/issues/3660
...
func (sw *scrapeWork) addAutoMetrics(am *autoMetrics, wc *writeRequestCtx, timestamp int64) {
	...
	if sampleLimit := sw.Config.SampleLimit; sampleLimit > 0 {
		...
		// See https://github.com/exsplashit/operator/issues/497
...
func (sw *scrapeWork) addRowToTimeseries(wc *writeRequestCtx, r *parser.Row, timestamp int64, needRelabel bool) {
	...
	//
	// See https://github.com/exsplashit/VictoriaMetrics/issues/3557
	// and https://github.com/exsplashit/VictoriaMetrics/issues/3406
