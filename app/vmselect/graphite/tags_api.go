...
import (
	...
	graphiteparser "github.com/exsplashit/VictoriaMetrics/lib/protoparser/graphite"
...
func TagsAutoCompleteValuesHandler(startTime time.Time, at *auth.Token, w http.ResponseWriter, r *http.Request) error {
	...
	if len(exprs) == 0 && len(etfs) == 0 {
		...
		tagValues, isPartial, err = netstorage.GraphiteTagValues(nil, at.AccountID, at.ProjectID, denyPartialResponse, tag, filter, *maxGraphiteTagKeysPerSearch, deadline)
		if err != nil {
			return err
		}
	} else {
